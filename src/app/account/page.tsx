...
import {
  ...
  Lock, Eye, Zap, TrendingUp, Clock, Settings, LogOut, Copy, Check, Store, Edit
} from "lucide-react";
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-md hover:bg-white/10 transition-all">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-white/50">{profileCompleteFields} of 5 fields complete</p>
              </div>
            </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl backdrop-blur-md overflow-hidden shadow-2xl">
                ...
                {isLoading ? (
                  ...
                  <div className="p-6 space-y-6">
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <p className="text-sm font-semibold text-white/60 mb-2">First Name</p>
                        <p className="text-lg text-white font-medium">{formData.firstName || '—'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-white/60 mb-2">Last Name</p>
                        <p className="text-lg text-white font-medium">{formData.lastName || '—'}</p>
                      </div>
                    </div>

                    <div>
                      <p className="text-sm font-semibold text-white/60 mb-2">Email Address</p>
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white/60 mb-2">Phone Number</p>
                      <p className="text-white/90">{formData.phone || '—'}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {activeTab === 'security' && (
              <div className="bg-white/5 border border-white/10 rounded-xl backdrop-blur-md overflow-hidden shadow-2xl">
                ...
                <div className="p-6 space-y-4">
                  {/* Email Verification */}
                  <div className="p-4 bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/30 rounded-lg flex items-start gap-4 hover:from-emerald-500/20 hover:to-teal-500/20 transition-all">
                    <div className="flex-shrink-0">
                      <CheckCircle2 className="w-6 h-6 text-emerald-400" />
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-white mb-0.5">Email Verified</p>
                      <p className="text-sm text-white/60 mb-3">{display.email}</p>
                      <p className="text-xs text-emerald-300 font-medium">✓ Verified and Active</p>
                    </div>
                  </div>

                  {/* Password Security */}
                  <div className="p-4 bg-white/5 border border-white/10 rounded-lg flex items-start gap-4 hover:bg-white/10 transition-all">
                    <div className="flex-shrink-0">
                      <Lock className="w-6 h-6 text-white/40" />
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-white mb-0.5">Password</p>
                      <p className="text-sm text-white/60 mb-3">Update your password regularly for security</p>
                      <button className="text-sm text-pink-400 hover:text-pink-300 font-semibold transition-colors flex items-center gap-1.5">
                  ...
                  <div className="p-4 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition-all">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="font-semibold text-white mb-0.5">Active Sessions</p>
                        <p className="text-sm text-white/60">1 active session</p>
                      </div>
                      <span className="px-2.5 py-1 bg-emerald-500/20 text-emerald-300 text-xs font-bold rounded-full border border-emerald-500/30">Active</span>
                    </div>
                  </div>

                  {/* User ID */}
                  <div className="p-4 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition-all">
                    <p className="text-sm font-semibold text-white/60 uppercase tracking-wider mb-2">Your User ID</p>
                    <div className="flex items-center gap-2 bg-black/20 rounded-lg p-3">
                      <code className="text-xs font-mono text-white/70 flex-1 truncate">{display.id}</code>
                      <button
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl backdrop-blur-md overflow-hidden shadow-2xl">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { label: 'Promotional Emails', desc: 'Receive special offers and promotions' },
                        { label: 'Product Recommendations', desc: 'Personalized product suggestions' },
                      ].map((pref) => (
                        <label key={pref.label} className="flex items-center p-3 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 cursor-pointer transition-all group">
                          <input type="checkbox" defaultChecked className="w-4 h-4 rounded bg-white/20 border-white/40 cursor-pointer accent-pink-500" />
                  ...
                  <div className="pt-4 border-t border-white/10">
                    ...
                    <div className="space-y-3">
                      {[
                        { label: 'Show Profile Publicly', desc: 'Allow others to see your profile' },
                        { label: 'Allow Personalization', desc: 'Use my data to personalize my experience' },
                      ].map((pref) => (
                        <label key={pref.label} className="flex items-center p-3 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 cursor-pointer transition-all">
                          <input type="checkbox" defaultChecked className="w-4 h-4 rounded bg-white/20 border-white/40 cursor-pointer accent-pink-500" />
                          <div className="ml-3 flex-1">
                            <p className="text-sm font-medium text-white">{pref.label}</p>
                            <p className="text-xs text-white/50">{pref.desc}</p>
                  ...
                  <div className="pt-4 border-t border-white/10 space-y-3">
                    <button className="w-full flex items-center gap-2 px-4 py-3 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 hover:border-red-500/50 text-red-400 font-semibold rounded-lg transition-all">
